...
const boats = sandbox.window.BOATBUILDER_DATA.items.filter(item => item.manufacturer === "Fisher");
fs.mkdirSync("reports", { recursive: true });
fs.writeFileSync("reports/fisher-current.json", `${JSON.stringify(boats, null, 2)}\n`);
const rows = [
  "# Fisher Current Audit Snapshot", "", `Generated ${new Date().toISOString()}.`, "",
...
fs.writeFileSync("reports/fisher-current.md", `${rows.join("\n")}\n`);
console.log(`Captured ${boats.length} Fisher records.`);
